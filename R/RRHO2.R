                   ...
                   log10.ind = FALSE, maximum=50, boundary = 0.1, res=100, method="fisher", alternative="split")
{
  ...
  if(alternative == "two.sided" | alternative == "enrichment"){
    .hypermat<- numericListOverlap(list1[, 1], list2[, 1], stepsize, method=method, alternative = alternative, maximum = maximum)
    ...
    if(plots) {
      try({
        ...
        .filename <-paste("RRHOMap", labels[1], "_VS_", labels[2], ".jpeg", sep="") 
        ...
        h1  <- draw.pairwise.venn(length(indlist1.ur:nlist1),
                                  ...
                                  cat.dist=0.01)
        grid.draw(h1)
        grid.text("Significant Overlapping",y=1)
        upViewport()
        pushViewport(vp2)
        h2  <-  draw.pairwise.venn(length(1:indlist1.lr),
                                   ...
                                   cat.dist=0.01)
        grid.draw(h2)
        grid.text("Non-significant Overlapping",y=1)
        dev.off()
      })
    ...
    if (plots) {
      try({
        ...
        finite.ind <- is.finite(hypermat)
        color.bar(jet.colors(101), min = min(hypermat[finite.ind],
                                             na.rm = TRUE), max = maximum, nticks = 6, title = "-log(P-value)")
        dev.off()   
        
        if(F){
          ## maximum with highest point
